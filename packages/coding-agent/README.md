...
Configure delivery in [settings](docs/settings.md): `steeringMode` and `followUpMode` can be `"one-at-a-time"` (default, waits for response) or `"all"` (delivers all queued at once). `askUserMode` controls how concurrent `ask_user` questions are surfaced — `"sequential"` (default, strict one-at-a-time FIFO) or `"tabbed"` (multiple questions open concurrently as switchable tabs). `transport` selects provider transport preference (`"sse"`, `"websocket"`, or `"auto"`) for providers that support multiple transports.
...
`ask_user` pauses the turn and asks you a structured clarifying question — Markdown-formatted question text, optional 2-4 multiple-choice options (single- or multi-select), and a "type your own answer" field — rendered natively in the TUI and the Dashboard, and over RPC. Answering submits the structured result; choosing **Stop agent** or pressing `Esc` aborts the whole current turn rather than continuing without an answer. Options must contain nonblank text. When there are no options the free-text field is always shown, so every question is answerable. An optional `timeoutSeconds` auto-stops the turn after the given number of seconds with a live countdown on every surface; recovered Dashboard sessions retain the original deadline rather than restarting it. Tool abort, timeout, host teardown, and headless/no-UI modes settle cleanly, so the agent never deadlocks waiting on an absent user. UI or response-protocol failures remain distinct from question closure. How concurrent `ask_user` calls are surfaced is controlled by the `askUserMode` [setting](docs/settings.md): `"sequential"` (default) shows them strictly one at a time (FIFO), while `"tabbed"` lets multiple questions open concurrently as switchable tabs — each keeps its own countdown and resolves independently, and the editor is restored only after the last one is answered. In the TUI: `↑`/`↓` move between options and the free-text field, `Space` toggles a checkbox (multi-select), `Enter` submits, `Esc` stops the turn, and in tabbed mode `Tab`/`Shift+Tab` switch between open questions. In the Dashboard: native radios/checkboxes plus a text field, an in-card **Stop agent** button that remains accessible on mobile, `Esc` to stop, and in tabbed mode a tab strip above the question card for switching between concurrent questions.
