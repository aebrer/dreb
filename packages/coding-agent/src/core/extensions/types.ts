...
import type { AgentMessage, AgentToolResult, AgentToolUpdateCallback, ThinkingLevel } from "@dreb/agent-core";
import type {
	Api,
...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@dreb/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@dreb/coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown dreb and exit. Available in all contexts. */
	shutdown(): void;
...
/** Configuration for registering a provider via dreb.registerProvider(). */
...
export type ExtensionFactory = (dreb: ExtensionAPI) => void | Promise<void>;
...
/**
 * Action implementations for dreb.* API methods.
