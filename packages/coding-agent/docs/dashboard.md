...
- **Theme gallery and font selection.** The settings appearance section shows
  ...
  IBM Plex Mono, JetBrains Mono, and the bundled dyslexia-friendly OpenDyslexic
  can also be selected independently. Explicit
...
- **Font loading.** Most themes default to Google-hosted IBM Plex Mono; Gruvbox
  ...
  `src/client/assets/fonts/`). The bundled OpenDyslexic (OFL, same directory)
  is an explicit dyslexia-friendly option, and any explicit font selection
  overrides the theme default. The self-hosted families are lazy-loaded only
