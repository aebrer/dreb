...
- **Theme gallery and font selection.** The settings appearance section shows
  ...
  IBM Plex Mono and JetBrains Mono can also be selected independently. Explicit
...
- **Font loading.** Most themes default to Google-hosted IBM Plex Mono; Gruvbox
  defaults to self-hosted JetBrains Mono (OFL, provenance in
  `src/client/assets/fonts/`). An explicit font selection overrides any theme.
  JetBrains Mono is lazy-loaded only when active typography uses it. No
  `light-dark()` is used, keeping an iOS Safari 16.4 floor.
