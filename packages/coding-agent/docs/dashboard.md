...
authoritative reload. Non-allowlisted or malformed image blocks stay inline and
are rejected by the dashboard's strict decode exactly as before, and command
responses (`get_messages`, `get_dashboard_snapshot`) always carry full payloads
because they are the authoritative source image recovery reads.
