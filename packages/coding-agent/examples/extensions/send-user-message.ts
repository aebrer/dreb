 ...
 *
 * Demonstrates dreb.sendUserMessage() for sending user messages from extensions.
 * Unlike dreb.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (dreb: ExtensionAPI) {
	...
	dreb.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			dreb.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	dreb.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				dreb.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	dreb.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				dreb.sendUserMessage(args, { deliverAs: "followUp" });
