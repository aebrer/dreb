...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@dreb/coding-agent";

export default function (dreb: ExtensionAPI) {
	dreb.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
