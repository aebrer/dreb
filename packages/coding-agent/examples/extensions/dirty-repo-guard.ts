...
async function checkDirtyRepo(
	...
	const { stdout, code } = await dreb.exec("git", ["status", "--porcelain"]);
...
export default function (dreb: ExtensionAPI) {
	dreb.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(dreb, ctx, action);
	});

	dreb.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(dreb, ctx, "fork");
	});
}
