 ...
 *
 * Usage:
 *   dreb -e ./ssh.ts --ssh user@host
 *   dreb -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (dreb: ExtensionAPI) {
	dreb.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	dreb.on("session_start", async (_event, ctx) => {
		...
		const arg = dreb.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
