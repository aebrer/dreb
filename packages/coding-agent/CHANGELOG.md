...
- **`get_settings` and `set_settings` RPC commands** - Typed settings read/write surface for external frontends (dashboard settings tab). `get_settings` returns a snapshot of the persistent defaults that seed fresh runtimes (default provider/model, default thinking level, steering/follow-up modes, compaction/retry toggles) — distinct from `get_state`'s live runtime view. `set_settings` takes a partial payload, validates every field before applying anything (atomic: unknown keys, bad enum values, non-boolean toggles, provider-without-model, and unavailable provider/model combos all reject loudly with nothing applied), persists via `SettingsManager`, and returns the post-write snapshot. Persistence failures are loud: a corrupt settings file (which makes saves silently no-op) and write I/O errors both surface as explicit `success: false` errors. `RpcClient` gains `getSettings()` and `setSettings()`. ([#314](https://github.com/aebrer/dreb/issues/314))
...
- **Streaming/compaction guard in `AgentSession.navigateTree`** - Navigating the session tree mid-stream, during compaction, or concurrently with another navigation now throws loudly instead of silently replacing agent messages or interleaving tree mutations. ([#313](https://github.com/aebrer/dreb/issues/313))
