...
```typescript
import { TUI, Text, Editor, ProcessTerminal } from "@dreb/tui";
...
```typescript
import { CombinedAutocompleteProvider } from "@dreb/tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@dreb/tui";
import type { Component } from "@dreb/tui";
...
```bash
DREB_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
