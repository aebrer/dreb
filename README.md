...
- **A real development workflow.** [mach6](packages/coding-agent/docs/mach6.md) is a built-in issue-to-merge workflow: assess issues, plan work, open draft PRs, implement, push progress, run multi-agent reviews, independently assess findings, fix CI or review items, and publish. Plans, reviews, and progress live on GitHub as shared memory. When a compatible local Graphify CLI is already installed, mach6 can use bounded AST-only structural evidence during assessment, planning, and review; it remains supplemental to direct source and test inspection and is never required or installed automatically.
