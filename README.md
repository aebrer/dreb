...
Main-session and subagent transcripts use separate roots. `sessionDir` (or CLI `--session-dir`) selects a flat main-session store; CLI precedence is flag, merged global/project settings, extension hook, then the built-in per-project tree. Dashboard host-wide inventory reads only the global `sessionDir` at startup, so restart Dashboard after changing it. Global-only `subagentSessionDir` controls new child logs; restart-time recovery also reads the legacy `~/.dreb/agent/subagent-sessions` root when different. Absolute paths remain absolute, `~` expands from the home directory, and relative paths use the relevant CLI/Dashboard/AgentSession startup cwd. Changing either root does not migrate existing files or affect credentials or general configuration. See [session settings](packages/coding-agent/docs/settings.md#sessions).
...
**Fleet overview.** Home base is every session across every project: live sessions with status chips (running / needs-attention / idle / error), activity lines, running subagents, task progress, context usage, and model — plus past sessions grouped by project, resumable with one tap. Sessions copied from another host or whose recorded project directory has moved remain visible; if that historical path is unavailable, Dashboard asks for an existing runtime project directory and keeps both paths visible without rewriting the session header. Terminal provider/API failures show their reason on the fleet card; transient failures clear that terminal card state when automatic retry begins. Initial load fetches live cards and opens SSE before the historical inventory scan finishes; unchanged session metadata is reused on later inventory and recovery reads. Live cards then update through compact SSE snapshots instead of re-fetching the full cross-project inventory on every turn, keeping weak mobile links responsive. When a session needs input, the browser tab badges and (opt-in) sends a service-worker notification (installable PWA, works on Android and iOS). For opt-in local mobile transport profiling, see the [dashboard docs](packages/coding-agent/docs/dashboard.md#mobile-transport-profiling).
